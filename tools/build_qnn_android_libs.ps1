param(
    ...
    [string]$InputRoot = (Join-Path (Split-Path -Parent $PSScriptRoot) "build\qnn_text_encoder_segments"),
    [string]$OutputRoot = (Join-Path (Split-Path -Parent $PSScriptRoot) "build\android\jniLibs\arm64-v8a"),
    [string[]]$Segments = @('embedding','layers_00_05','layers_06_11','layers_12_17','layers_18_23','layers_24_29','layers_30_35'),
    [switch]$ForceRebuild
)
...
foreach ($segment in $Segments) {
    ...
    $outLib = Join-Path $OutputRoot "libqnn_$segment.so"
