param(
    ...
    [string]$OutputRoot = (Join-Path $PSScriptRoot "..\build\qnn_transformer_segments_v3_floatio_w4a8"),
    [string]$FrontendCalibration = "E:\zimage_calib_transformer_frontend_bool\input_list_4.txt",
    [string]$ChainCalibration = "E:\zimage_calib_transformer_chain",
    [string]$Python = $env:QAIRT_PYTHON,
    [switch]$Force
)

# Transformer W4A8 with FLOAT graph I/O (--preserve_io datatype).
#
# The v2 segmented build exposed two problems:
#   1. segment-boundary activations were quantized with per-segment random
#      calibration, so unified_in/u8 encodings did not match the previous
#      segment's real output distribution;
#   2. UFIXED_POINT_8 segment IO would have forced CPU dequant/requant and a
#      full extra pass over ~24 MB at every boundary.
# Keeping IO as float16/float32 makes the graph insert its own Convert nodes,
# while all internal activations/weights remain W4A8. Calibration for layer
# groups comes from chained ONNX runs (E:\zimage_calib_transformer_chain).
#
# Note: frontend cap_mask raw files must be uint8/bool when IO dtype is
# preserved; frontend unified_freqs is constant-folded away by the converter,
# so the runtime must supply it from build/transformer_freqs_qnn.
...
function Convert-One($Name, $InputOnnx, $OutputDir, $InputList, $Dims) {
    ...
    $args += @("--weights_bitwidth","4","--act_bitwidth","8","--pack_4_bit_weights","--preserve_io","datatype")
...
Convert-One "frontend" (Join-Path $SegmentsDir "frontend\model.onnx") (Join-Path $OutputRoot "frontend") $FrontendCalibration @(
...
Write-Host "DONE Transformer v3 float-IO W4A8 under $OutputRoot"
